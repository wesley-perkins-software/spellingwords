...
- **Themed gateway** — explains the *purpose* of themed practice (a recognizable, coherent context for additional spelling practice, not a curriculum) and helps a parent or teacher *choose* a theme; it owns "extra/fun/themed spelling practice for [grade]" intent, deliberately not competing with Core for "curriculum" framing.

All three gateways should add genuine information gain over their member pages — synthesis across units/sets/themes, not a copy-paste of child-page descriptions — while staying substantially shorter than a member page and treating the complete list of practice destinations (the child links) as a primary, required page function, independent of visual presentation. The research recommends a **shared four-part skeleton** (orientation/answer, synthesis, complete crawlable navigation, cross-strand/grade wayfinding) with strand-specific content inside each part, rejects FAQ quotas and AI-specific gimmicks as unsupported by evidence, and recommends piloting **all three Kindergarten gateways together** so the differentiated model can be validated across all three strands in one grade before scaling to Grades 1–5.
...
- Themed practice groups words into a **recognizable, coherent context** (animals, weather, community helpers, etc.) that a parent or child can immediately identify and relate to. This document does **not** claim that thematic grouping measurably improves spelling accuracy, retention, or retrieval — no source found in this research directly establishes that effect for spelling specifically, and the general word-study/retrieval-practice literature cited above is about *how words are practiced* (hearing, mapping, producing from memory), not about *whether grouping by theme* changes the outcome of that practice. The defensible, source-supported claim is narrower: a theme provides a coherent, motivating *organizing frame* for a bounded word list; whether it also confers a retrieval advantage is an open question this document does not resolve and does not assert.

**What this document does NOT claim:** any specific percentage improvement from a gateway page, any specific "optimal" number of FAQs, any claim that themed practice is empirically superior to unthemed practice for spelling specifically, and — revised from an earlier draft of this document — no claim that "themed context helps spelling/retrieval." Earlier language in this document to that effect has been corrected throughout (§1, §15, §16, §27, and the closing answers) to describe themes only as recognizable, coherent contexts for additional practice, not as a mechanism with an established retrieval or retention benefit. Any such stronger claim in a future draft should be flagged and either sourced or removed.
...
| Student: "What can I practice? Where do I click?" | The complete member list itself — must remain a primary, required function of the page (not an optional or secondary element relative to any authored prose), regardless of how a future redesign positions it visually (§12, §22) |
...
**Search-observation grounding — first-party, distinguished from product inference:** Google's own developer documentation, not third-party SEO commentary, is the authoritative source for what actually matters here, and it supports a narrower, more defensible claim than "hub pages outrank detail pages." Google's Search Essentials and SEO Starter Guide state directly that organizing content logically — including using directories/sections to group similar topics — helps both crawlers and users understand how pages relate to the rest of a site, and that navigation structures (menus, cross-page links) affect how Google infers the relative importance of a page ([Google Search Essentials](https://developers.google.com/search/docs/essentials); [SEO Starter Guide: The Basics](https://developers.google.com/search/docs/fundamentals/seo-starter-guide); [URL Structure Best Practices for Google Search](https://developers.google.com/search/docs/crawling-indexing/url-structure)). Google's structured-data documentation for `BreadcrumbList` similarly confirms that breadcrumb markup helps Google understand a page's position in site hierarchy and can improve how a URL is represented in search results ([How To Add Breadcrumb (BreadcrumbList) Markup](https://developers.google.com/search/docs/appearance/structured-data/breadcrumb)).

**What this first-party sourcing directly supports, and what it does not:** it directly supports that a gateway layer sitting between the Grade Hub and member pages, linked via real navigation and represented via accurate `BreadcrumbList`/`ItemList` structured data, helps Google understand the site's hierarchy and the gateway's place in it — this is the case already made independent of the gateway's *editorial* content, in §3 and §20. It does **not** directly support a specific ranking or traffic-volume claim ("hub pages outperform detail pages by X") — that stronger claim, found in secondary SEO-vendor commentary during initial research, is removed from this document as unsupported by any first-party source located. The defensible, source-grounded claim is narrower and is the one this document relies on: a strand gateway that genuinely organizes and orients (rather than acting as a thin doorway page) is consistent with Google's own stated crawling/understanding guidance, and is a legitimate, distinct asset from its member pages on that basis — not because it is asserted to out-rank them.
...
- **First-party, primary source:** Google's own official documentation for optimizing for generative AI features states directly that no special machine-readable files, AI text files, markup, or Markdown are needed to appear in Google Search, including its generative AI capabilities, "as Google Search itself doesn't use them" ([Google's Guide to Optimizing for Generative AI Features on Google Search](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide)). This is Google's own documentation, not a third party's summary of it, and it is the strongest single piece of evidence in this section.
- **Primary statements, reported by trade press:** Google's Gary Illyes stated at Google Search Central Live that Google does not support `llms.txt` and has no plans to; Google's John Mueller separately compared it to the deprecated keywords meta tag — a site-owner's self-declared claim about what a site covers, rather than something Google verifies against the actual site ([Google says normal SEO works for ranking in AI Overviews and LLMS.txt won't be used, Search Engine Land](https://searchengineland.com/google-says-normal-seo-works-for-ranking-in-ai-overviews-and-llms-txt-wont-be-used-459422); [Google Search Team Does Not Endorse LLMs.txt Files, Search Engine Roundtable](https://www.seroundtable.com/google-does-not-endorse-llms-txt-40789.html)). These are direct quotations of Google personnel, reported by established search-industry trade press, not vendor marketing claims about llms.txt's supposed benefits — the distinction the task's skepticism requirement asks this document to maintain.
- **Conclusion: do not build `llms.txt`-based content decisions for these gateways.** This is now grounded in Google's own first-party documentation (the strongest form of evidence available), directly answering the task's own prohibition (§9), and corroborated by reported primary statements from named Google search personnel — not by third-party "AI SEO" vendor commentary, which this document explicitly excludes as a sourcing basis per the task's skepticism instruction.
...
5. **Keep the complete member list a primary, required function of the page, not a secondary one** — this is a presentation-independent requirement, not a layout instruction: whatever a future visual redesign does with placement, spacing, or ordering, the list must remain complete, unabbreviated, and structurally central to the page's purpose. No amount of synthesis prose may make the list optional, partial, or presented as less important than the prose (explicit anti-pattern, §23).
...
- Brief framing of **what themed practice is** — a recognizable, coherent context (a familiar topic or setting) for an additional, optional round of spelling practice — stated once, modestly, and without asserting a retrieval or retention benefit that no source in this research establishes (§6). The claim is about *coherence and recognizability*, not about learning outcomes.
...
**Smallest viable path for the pilot (recommended, not frozen):** add authored copy analogous to the existing `gradeHubCopy.ts` pattern (already used for Grade Hub hero copy) — e.g., a new `gradeStrandGatewayCopy.ts` module keyed by `(grade, strand)` holding the orientation sentence, the synthesis paragraph(s), and any conditional fields (starting-point pointer, generated cross-strand link target per §21). This:
...
**Important qualification: this is the recommended *pilot* implementation choice, not a permanently frozen part of the semantic standard.** The semantic standard this document proposes (§27) is the required/conditional/shared content *model* — what each gateway must say and in what order. `gradeStrandGatewayCopy.ts` is one reasonable, smallest-viable *data shape* for holding that content at 3-gateway pilot scale; it is not itself a frozen architectural requirement the way the route manifest or content-collection schema is. The pilot should explicitly test whether this module shape remains sufficient before Grades 1–5 scale the total to 18 entries — in particular whether a flat `(grade, strand)`-keyed object stays readable and reviewable at that size, whether the conditional fields (starting-point pointer, cross-strand link inputs) need a more structured shape once every grade's real content is in hand, and whether editorial review of 18 entries in one TS file is still comfortable compared to some other authoring surface. If the pilot surfaces friction, revisiting the data-model choice (without revisiting the semantic standard itself) is expected, not a standard violation.

**Alternative considered and not recommended for the pilot:** a new Content Collection or per-gateway Markdown file. Rejected because 18 short, tightly-parameterized pages don't need full Markdown/Content-Collection ceremony (frontmatter schema, content body parsing) when a typed TS object serves the same purpose with less machinery — consistent with "prefer the smallest viable implementation" (task §19) and the codebase's own precedent (`gradeHubCopy.ts` already made this exact choice for a structurally similar problem). This remains a live option to reconsider at Grade 1–5 scale if the pilot shows the TS-module shape doesn't hold up, per the qualification above.
...
### 20.1 Public grade-label convention (visible labels only)

The site already has an approved public grade-label convention, stated for member-page titles in `CANONICAL_THEMED_SPELLING_PRACTICE_PAGE_STANDARD.md` §10: **Kindergarten, 1st Grade, 2nd Grade, 3rd Grade, 4th Grade, 5th Grade.** That standard flags the *member-page title* application of this convention (`[Public grade label] [Theme] Spelling Words`) as directional, with a note that "a later coordinated task must reconcile Core, HFW, Themed, Grade Hubs, gateways, breadcrumbs, browser titles, navigation, and visible structured-data names" — i.e., full cross-site reconciliation is separately deferred and this document does not resolve it.

What this document does add: the strand gateways should **already** use these exact labels, because they do today. `gradeConfig.ts`'s `label` field is defined verbatim as `Kindergarten`, `1st Grade`, `2nd Grade`, `3rd Grade`, `4th Grade`, `5th Grade`, and the current gateway renderer (`[strand].astro`) already composes its H1, title tag, and breadcrumb entirely from `gradeEntry.label`. The proposed standard in §27 should **explicitly preserve this**, not silently assume it: any authored orientation/synthesis copy for a gateway must refer to the grade using this exact public label (never an internal code like `K` or `grade-1`, never an invented variant), consistent with the existing `label` field and with the Themed standard's own stated direction for public grade labels elsewhere on the site.

**Scope of this clarification, explicitly:**
- This concerns **visible labels only** — what a reader sees in an H1, title tag, breadcrumb, or authored sentence.
- It does **not** change any internal `GradeCode` value (`'K' | '1' | '2' | '3' | '4' | '5'`), any route slug (`kindergarten`, `1st-grade`, etc.), any stable content `id`, or any canonical path.
- It does **not** implement the broader cross-site title reconciliation the Themed standard flags as a separate future task (member-page titles, Grade Hub copy, and any other surface using a grade label are out of scope here and remain deferred exactly as that standard already states).
- Since the gateway renderer already sources its grade label from `gradeConfig.ts.label`, no code change is implied by this clarification — it is a confirmation that new authored gateway copy must stay consistent with what the renderer already does, not a proposal to change how grade labels are generated.
...
- **Add, capped and judged case-by-case:** same-grade cross-strand links (e.g., Core K → HFW K, Core K → Themed K). **These must be lightweight same-grade wayfinding only** — a link with a short, factual label (e.g., "This grade's High-Frequency Words") — **never a restatement of what that strand is or why it matters.** That explanation is the Grade Hub's job, already covered in §11.1's boundary row for cross-strand awareness: the Hub owns the full three-way explanation, the gateway's cross-strand links may only point to it, not duplicate it. Evaluate whether to include each link with the same "reasonable-parent test" already codified for member pages in `CANONICAL_NAVIGATION_RELATIONSHIPS.md` — would a parent who just read this gateway's synthesis naturally want to know "what else does this grade offer?" Plausibly yes, especially from Core (the default path) toward HFW/Themed (supplementary), less obviously useful in the reverse direction. Recommend: Core gateway may link to its grade's HFW and Themed gateways; HFW/Themed gateways link back to Core (as "the main sequence") and to each other only if genuinely useful — decide per-gateway during implementation, not force symmetric linking for its own sake (explicit prohibition already established at the member-page level and equally applicable here).
- **Preferably generated, not manually authored per gateway.** Because the three strands and their route segments are already a canonical, closed set — `GRADE_STRANDS` in `src/lib/content/canonicalGradeRoutes.ts` enumerates exactly `core-spelling`, `high-frequency-words`, and `themed-spelling-practice` for every grade, and `getGradeStrandPath(grade, strand)` already computes any gateway's URL from that same manifest — a cross-strand link's *label and destination* should be **derived from this existing canonical data at render time**, not hand-typed into 18 separate authored-copy entries. This keeps the link set correct by construction (adding or renaming a strand automatically updates every gateway's cross-strand links) and prevents the exact kind of manual drift the site's existing standards spend real effort avoiding elsewhere (e.g., the broken-link/stale-reference issues already found and fixed in `CANONICAL_NAVIGATION_RELATIONSHIPS.md`). Only the *decision of which cross-strand links to show at all* (§18's 0–2 cap, the reasonable-parent judgment above) needs to be an authored/editorial choice per strand; the resulting link's path and visible label should be computed, not typed by hand. This is a recommendation for the implementation phase, consistent with the "smallest viable implementation" principle already applied in §19 — it does not change the semantic standard itself (0–2 links, same-grade only, wayfinding-only labels), only how those links get built.
...
- **The complete member inventory's primacy is a functional requirement, not a layout instruction.** The standard requires that every gateway's complete, unabbreviated list of member pages remain a primary, required function of the page — never optional, partial, or presented as secondary to authored prose — but it does not require any particular visual position, ordering, or size for that list. A future redesign is free to place, style, or arrange the list however it chooses, as long as it stays complete and remains one of the page's primary functions rather than an incidental one. This document deliberately avoids phrases like "above the fold" or "first thing the reader sees," since those describe a specific layout, not the underlying semantic requirement.
...
- Giant SEO essays before curriculum links — rejected; §12 requires the complete child list remain a primary, required page function regardless of presentation.
...
- Making the practice-destination list optional, partial, or secondary to explanatory prose in any presentation — rejected in §12 (component 5).
...
| H1 | Identity | Renderer-owned (composed from grade+strand labels; grade label uses the existing public convention, see §20.1) | Required | 1 line | Mechanical | — | — |
...
| Synthesis (strand-specific, see below) | Information gain across the strand | **Authored** | Required | 1 short paragraph (~2–4 sentences) | Yes | 0–2, capped, same-grade, lightweight wayfinding only — generated from `GRADE_STRANDS`/`getGradeStrandPath`, not hand-authored per gateway (§21) | Member-page content; the Grade Hub's own cross-strand explanation |
...
- Cross-strand link: a generated, lightweight wayfinding link (label + destination computed from `GRADE_STRANDS`, not authored prose) to this grade's HFW and Themed gateways (Core is the natural jumping-off point). See §21.
...
- Cross-strand link: a generated, lightweight wayfinding link back to Core (as the grade's main sequence); a Themed link, also generated, is optional. See §21.
...
- Synthesis: 2–3 sentences describing what themed practice is — a recognizable, coherent context for additional spelling practice (measured, not overclaimed per §6; do not assert a retrieval/retention benefit) — and a light, descriptive grouping of the actual theme titles to aid selection (not a new taxonomy — just organizing what exists).
- No "start here" pointer (contradicts non-sequential framing).
- Cross-strand link: a generated, lightweight wayfinding link back to Core; an HFW link, also generated, is optional. See §21.
...
2. A decision on the pilot's data model (§19 recommends a new `gradeStrandGatewayCopy.ts`-style module as the smallest viable choice for 3 pilot entries; confirm before writing code). This is a pilot implementation choice, not part of the frozen semantic standard (§27) — the pilot should explicitly note whether this shape held up well enough to keep, or whether a different shape is warranted before Grades 1–5 bring the total to 18 entries.
3. Draft orientation + synthesis copy for the 3 Kindergarten gateways specifically (Core, HFW, Themed) — writing this is itself editorial work, not covered by this research task, and should be done with the same care as the 105 deepest pages (draft → self-review → adversarial review, per the pattern already established in `CONTENT_IMPROVEMENT_ROADMAP.md`), using the public grade-label convention already in `gradeConfig.ts` (§20.1) and describing themed practice per the corrected, non-overclaiming framing in §6/§15.
4. A decision on whether to add a gateway-rendering test (recommended, given §3's finding that no such test exists today) — e.g., asserting each of the 3 piloted pages renders its required components and that the child list stays complete regardless of layout.
...
6. A decision on how the pilot implements same-grade cross-strand links (§21) — recommended as generated from `GRADE_STRANDS`/`getGradeStrandPath` rather than hand-authored per gateway; confirm this is feasible within the pilot's chosen data model before writing gateway copy that assumes it.
...
1. **What is the unique purpose of a grade-level strand gateway?** To fill the one genuine gap between the Grade Hub (correctly thin, three-strand-wide) and the member pages (correctly narrow, single-unit) — synthesizing how one strand's member pages relate to each other as a group (sequence for Core, cumulative inventory for HFW, purpose/selection for Themed) — while providing complete, primary navigation into every member page. It is not a lesson, not a second Grade Hub, and not a thin index. See §11 for the full structural argument that this gap is real, not manufactured.
...
5. **What should Themed gateways contain that Core/HFW should not?** An explicit statement that themes are optional and non-sequential, a modest description of what themed practice is (a recognizable, coherent context for additional practice — not a claimed retrieval or retention benefit, §6), and a selection aid organizing the actual theme titles descriptively. See §15.
...
7. **How do we keep the actual practice destinations prominent?** By treating the complete member list as a primary, required function of the page — a presentation-independent requirement (§12 rule 5, §22), not a layout instruction. The list must remain complete and structurally central regardless of how a future visual redesign positions it; no word-count or component addition may make it optional, partial, or secondary to authored prose.
...
14. **What schema/renderer changes, if any, are actually necessary?** No Content Collection or Zod schema change. The recommended pilot data model is a new small TypeScript copy module (parallel to the existing `gradeHubCopy.ts` pattern) holding per-grade-per-strand orientation/synthesis copy, consumed by the existing shared `[strand].astro` renderer, which already branches per strand for its one sentence today. This module shape is a pilot implementation recommendation, not a frozen requirement of the semantic standard itself — the pilot should validate whether it remains sufficient before Grades 1–5 are scaled to it. See §19.
...
**SEO / search architecture — first-party Google documentation (primary sources, used for the ownership/structure claims in §8):**
- Google for Developers, [Google Search Essentials](https://developers.google.com/search/docs/essentials)
- Google for Developers, [SEO Starter Guide: The Basics](https://developers.google.com/search/docs/fundamentals/seo-starter-guide)
- Google for Developers, [URL Structure Best Practices for Google Search](https://developers.google.com/search/docs/crawling-indexing/url-structure)
- Google for Developers, [How To Add Breadcrumb (BreadcrumbList) Markup](https://developers.google.com/search/docs/appearance/structured-data/breadcrumb)

**GEO/AEO (skeptical review, first-party primary source plus trade-press reporting of named Google personnel — no "AI SEO" vendor commentary used):**
- Google for Developers, [Google's Guide to Optimizing for Generative AI Features on Google Search](https://developers.google.com/search/docs/fundamentals/ai-optimization-guide) — first-party: states directly that Google Search does not use special AI text files/markup
- Search Engine Land, [Google says normal SEO works for ranking in AI Overviews and LLMS.txt won't be used](https://searchengineland.com/google-says-normal-seo-works-for-ranking-in-ai-overviews-and-llms-txt-wont-be-used-459422) — reports Gary Illyes' statement at Google Search Central Live
- Search Engine Roundtable, [Google Search Team Does Not Endorse LLMs.txt Files](https://www.seroundtable.com/google-does-not-endorse-llms-txt-40789.html) — reports John Mueller's keywords-meta-tag comparison
