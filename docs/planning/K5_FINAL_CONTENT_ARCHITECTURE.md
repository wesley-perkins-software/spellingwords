...
| Grade 5 | Core Spelling implementation-complete; High-Frequency Words and Additional Practice planned | Five Core Spelling cards, one of them a combined roadmap entry reusing an existing sibling page, one new closing page (Section 14). High-Frequency Words (4 sets) and Additional Practice remain a separate, later implementation phase. |
...
| 5 | Advanced Multisyllabic Academic Words; Advanced Prefixes and Suffixes (merged with Spelling Rules for Word Endings, see Section 14); Greek and Latin Word Parts; Commonly Confused Words; Spelling Changes in Related Words (new, closing/capstone — see Section 14 for why the capstone slot is morphology, not the accuracy card). | Planned: 4 sets + gateway (separate implementation phase). | Planned, separate implementation phase: Math; Science; U.S. History/Civics; research/transition writing after audit. |
...
Grade 5's Core Spelling section also now realizes the hub pattern: five cards, one a combined roadmap entry, one wholly new closing page (Section 14). Grade 5 has no High-Frequency Words or Additional Practice section yet — that is a deliberately separate, later implementation phase (Section 14), so the Grade 5 hub currently renders one section, not three; this is an intermediate, not a final, state.
...
### Contract for Grade 5's remaining phase (High-Frequency Words and Additional Practice)
...
6. Curate Grade 5's Common Words sequence (4 sets) and Additional Practice — a separate, later implementation phase (Section 14).
7. Add optional topic cards only after each grade’s core route and Common Words system are complete and the topics clear editorial review.
8. Keep Kindergarten, Grade 1, Grade 2, Grade 3, Grade 4, and Grade 5 synchronized with implementation when maintenance changes their technical contracts; do not reopen their frozen architecture without an explicit architecture decision.
...
**Advanced Prefixes and Suffixes decision:** rather than split prefixes and suffixes into two cards the way Grade 4 does (Advanced Prefixes; Advanced Suffixes and Final Stable Syllables), Grade 5 realizes its canonical "Advanced Roots, Affixes, and Academic Words" unit as one combined affix card, `grade-5-prefix-suffix-words`, already teaching both prefixes and suffixes together. `grade-5-spelling-rules` (the -tion/-sion/-able/-ible/-ance/-ence endings) is marked `contentRole: skill` and reached through the anchor's related-practice link — the same merge mechanic used for Grade 4's Advanced Suffixes and Final Stable Syllables (Section 8) and Grade 3's Suffix Spelling Changes (Section 7). Both pages already existed, already grade-5-tagged, and already cross-referenced each other — no new content was needed, only `contentRole`, `skillIds`, and hub registration.
...
**Deferred to the next Grade 5 phase:** Common Words (4 sets, none published yet) and Additional Practice (`grade-5-science-nature-words`, `grade-5-community-civics-words`, `grade-5-math-vocabulary` are strong, non-placeholder candidates per Section 3's flexible row, but remain unwired pending that phase's own research and implementation pass).
