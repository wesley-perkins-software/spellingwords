...
> **Grade Hub and Gateway architecture correction:** this document's flattened Hub-card and legacy gateway model is retained only as implementation history. Current authority is `docs/content/CANONICAL_GRADE_HUB_STANDARD.md`, `docs/content/CANONICAL_GRADE_STRAND_GATEWAY_STANDARD.md`, and `docs/architecture/PUBLIC_URL_ARCHITECTURE.md`: each of the six Hubs routes to exactly three same-grade Gateways, and those 18 Gateways—not the Hubs—own the complete member inventories. The `spelling-collections` type and URL shape below were removed. The three-strand curriculum classification and per-grade member inventories remain valid; statements below that members appear directly on a Hub, that a standalone Gateway does not exist, or that the flattened Hub realization is final are superseded. Production Hub conformance remains pending, beginning with a 3rd Grade pilot.
> **August 2026 HFW supersession:** The frozen High-Frequency Words curriculum is now 27 sets / 316 unique words in a 4/7/7/5/2/2 grade distribution. Its exact inventory and ownership/overlap principles are governed by [`FROZEN_HIGH_FREQUENCY_WORDS_CURRICULUM.md`](../curriculum/FROZEN_HIGH_FREQUENCY_WORDS_CURRICULUM.md) and `src/lib/content/hfWordsCurriculum.ts`. All older 29-set/340-word High-Frequency Words tables and zero-overlap rationales below are retained only as implementation history and are not current curriculum authority. Current Grade-curriculum totals are 51 Core + 27 HFW + 27 Additional Practice = 105 pages.
...
> **Final Core bottom navigation:** the 51 Core Spelling Grade Units form one continuous Kindergarten–Grade 5 chain in canonical hub order. `CORE_SPELLING_SEQUENCE` is the sole navigation source: previous unit → **Review first**, next unit → **Next step**. Core pages never render **Explore more**; only the first unit lacks Review first, and only the Grade 5 capstone lacks Next step. High-Frequency Words and Additional Practice use the finalized non-Core models above.

> URL architecture update: canonical K–5 Grade Hub card URLs use the no-trailing-slash `/{grade}/{strand}/{page-slug}` structure, and canonical Skill pages use `/skills/{skill-slug}`. The runtime sources of truth are `src/lib/content/canonicalGradeRoutes.ts` and `src/lib/content/canonicalSkillRoutes.ts`; see `docs/content/inventory/grade-url-migration-map.md` for the grade-curriculum reference.
...
| Grade 2 | Implementation-complete | Thirteen Core Spelling cards, six direct High-Frequency Words sets (72 unique words), and five Additional Practice cards. |
| Grade 3 | Implementation-complete | Seven Core Spelling cards, five direct High-Frequency Words sets (60 words), and four Additional Practice cards — Map & Globe Words, Life Cycle Words, Time Words, and Multiplication & Division Words (Section 7). |
...
2. **High-Frequency Words** — cumulative high-frequency-word sets with phonics-first Heart Word guidance.
...
- A practice set remains a `spelling-lists` entry, in the existing `high-frequency-words` category for High-Frequency Words. This preserves current routes and requires no new top-level category.
- Keep stable IDs forever. A title, section placement, or helper copy can change without changing an ID or URL.
- `grade-unit` is for an instructional roadmap page; `skill` is reusable focused pattern practice; `high-frequency-word-set` is the appropriate existing content role for a numbered High-Frequency Words set; `vocabulary-theme` is for a bounded optional topic list.
- Do not create duplicate grade-nested URLs, duplicate High-Frequency Word pages, or duplicate Heart Word pages for the same words.

### High-Frequency Words system

Every grade has exactly one High-Frequency Words gateway and these required final numbered-set counts:
...
Dolch and Fry are source/reference pools, not public curriculum taxonomies. Historical list names, “high-frequency words,” and “Heart Words” are explanatory search and helper terms, never peer top-level architectures. Heart Word treatment is mainly K–2 and is an annotation, not a separate route family.

### What remains flexible

The following are deliberately **not** architecture decisions: exact word membership for the still-unimplemented Grade 4–5 sets, the detailed K/Grade 1/Grade 2/Grade 3 card descriptions, which later topic pages meet quality thresholds, the exact Grade 4–5 root families, and the individual visual treatment of an annotation. They require editorial curation or implementation work, but may not change the three sections, URL/identity rule, or High-Frequency Words set counts without an explicit new architecture decision.
...
The repository already contains a substantial raw library: published phonics, grade-level, high-frequency-word, and challenge lists; six grade hubs; Dolch collections; curated Kindergarten and Grade 1 progressions; reusable focused Skills; and one dynamic canonical detail route. The architecture work is therefore primarily **reclassification, linking, and selective gap filling**, not wholesale replacement.
...
| `spelling-collections` schema | Retain. Use it for High-Frequency Words gateways; no gateway-specific framework is needed. |
...
| Existing Heart Word pages | Retain as source material and related practice; absorb their public curriculum role into High-Frequency Words. |
...
|---|---|---|---|
| K | First Words; Short A, I, O, U, E; Mixed CVC Review; late-K Digraph preview. `-ck`/FLOSS are not required. | Four direct numbered sets on the hub; no standalone gateway page — the Grade Hub itself is the entry point for these High-Frequency Words sets. | Number Words; Color Words; Animal Words. |
| 1 | Short Vowel Review & C/K; FLOSS; Digraphs & Final `-ck`; Beginning Blends; Ending Blends; Silent E; Open Syllables & Final Y; Vowel Teams; `-s/-es`; `-ed/-ing`; R-Controlled Vowels; `-tch/-dge`. | Six direct numbered sets (72 unique words) on the hub; no standalone gateway page — the Grade Hub itself is the entry point for these High-Frequency Words sets. | Number Words 11–20; Days of the Week; Five Senses Words. |
| 2 | Long E EE/EA; Long I IE/IGH; Diphthongs OI/OY; Diphthongs OU/OW; Two Sounds of OO; Vowel Patterns AU/AW; R-Controlled ER/IR/UR; Soft C and Soft G; Two-Syllable Words; Words Ending in -le; Silent Letter Words; Compound Words; Contractions. | Six direct numbered sets (72 unique words) on the hub; no standalone gateway page — the Grade Hub itself is the entry point for these High-Frequency Words sets. | Transportation Words; Money Words; Number Words 20–100; Community Helpers; Months of the Year. |
| 3 | Longer Words and Syllable Division; Prefixes; Suffixes; Suffix Spelling Changes (merged, see Section 7); Plurals, Possessives, and Contractions; Homophones and Commonly Confused Words; Root Word Families (review). | Five direct numbered sets (60 words) on the hub; no standalone gateway page — the Grade Hub itself is the entry point for these High-Frequency Words sets. | Map & Globe Words; Life Cycle Words; Time Words; Multiplication & Division Words — see the Additional Practice decision in Section 7. |
| 4 | Advanced Multisyllabic Words; Advanced Prefixes; Advanced Suffixes and Final Stable Syllables (merged, see Section 8); Greek and Latin Roots (merged, see Section 8); Commonly Confused Words; Derived Words and Word Meaning (new, closing/bridge). | Four direct numbered sets (48 words) on the hub; no standalone gateway page — the Grade Hub itself is the entry point for these High-Frequency Words sets (Section 8). | Measurement Words (customary units only) (Section 8). |
...
The prior 180-page recommendation is not a target. Reuse existing canonical practice URLs and add only coherent pages: 29 High-Frequency Words sets, 6 High-Frequency Words gateways, a small number of genuine instructional gaps per remaining grade, and later optional topic pages that clear editorial review. Kindergarten, Grade 1, Grade 2, and Grade 3 now provide 21 published sets and 4 gateways. Grades 4–5 High-Frequency Words pages remain future implementation work.
...
|---:|---|---|---|---|---|---|---|
| 1 | Learn in Order | **First Words** | Direct Grade Unit: `kindergarten-first-words` → `/grades/kindergarten/first-words` | Existing mixed familiar CVC list. | Implemented. | Start with eight familiar, sound-out words that help a child connect spoken sounds to letters before concentrating on one vowel pattern. | Essential |
...
| 8 | Learn in Order | **Digraph Words** | Direct late-K preview Grade Unit: `kindergarten-consonant-digraphs` → `/grades/kindergarten/consonant-digraphs` | Existing K preview; link onward to canonical CH/SH/TH/WH Skills where available. | Implemented. | Try common two-letter consonant sounds after CVC work; this is a preview, not a required kindergarten milestone. | Optional preview |
| 1–4 | High-Frequency Words | **Kindergarten High-Frequency Words 1–4** | Direct canonical sets: `kindergarten-high-frequency-words-set-1` through `-4` | The four numbered K High-Frequency Words sets. | Implemented. | Four small cumulative sets of frequently encountered reading and writing words, with phonics and Heart Word guidance. | Essential |
| 1 | Additional Practice | **Number Words** | Direct vocabulary page: `kindergarten-number-words` → `/grades/kindergarten/number-words` | One through ten. | Implemented. | Practice useful number spellings from kindergarten math and classroom routines. | Optional |
| 2 | Additional Practice | **Color Words** | Direct vocabulary page: `kindergarten-color-words` → `/grades/kindergarten/color-words` | Red, blue, green, yellow, black, white, brown, pink. | Implemented. | Practice familiar color spellings from art, sorting, books, and classroom routines. | Optional |
| 3 | Additional Practice | **Animal Words** | Direct vocabulary page: `kindergarten-animal-words` → `/grades/kindergarten/animal-words` | Existing familiar animal list. | Implemented. | Practice concrete animal names while applying early sound-to-letter knowledge. | Optional |
...
The visible Grade 1 hub is implementation-complete. It has twenty-one cards: twelve Core Spelling cards, six direct High-Frequency Words set cards, and three Additional Practice cards. The `grade-1-high-frequency-words` collection remains the canonical explanatory gateway, but the hub exposes its six children directly so families can see the complete sequence.
...
| 1–6 | High-Frequency Words | **Grade 1 High-Frequency Words 1–6** | Direct canonical sets: `grade-1-high-frequency-words-set-1` through `-6` | Implemented | Practice 72 unique Grade 1 writing words in six cumulative, phonics-aware sets with Heart Word guidance. |
...
**High-Frequency Words decision:** show all six direct set cards rather than only the gateway. The gateway remains published for orientation, collection metadata, and a crawlable complete child list. The six sets contain 72 unique words; cumulative describes the instructional sequence, not duplicated membership.
...
The visible Grade 2 hub is implementation-complete with twenty-four cards: thirteen Core Spelling cards, six direct High-Frequency Words sets, and five Additional Practice cards. The `grade-2-high-frequency-words` collection remains the explanatory gateway while the hub exposes its six children directly.

| # | Section | Public title | Type and destination | Status | Parent-facing purpose |
|---:|---|---|---|---|---|
| 1 | Core Spelling | **Long E Vowel Teams: EE and EA** | Direct Grade Unit: `grade-2-long-e-ee-ea` → `/grades/2nd-grade/long-e-ee-ea` | Implemented | Contrast two common spellings for long e in familiar words. |
| 2 | Core Spelling | **Long I Patterns: IE and IGH** | Direct Grade Unit: `grade-2-long-i-ie-igh` → `/grades/2nd-grade/long-i-ie-igh` | Implemented | Extend long-i spelling beyond silent e and final y. |
| 3 | Core Spelling | **Diphthongs: oi and oy** | Direct Grade Unit: `vowel-teams-oi-oy` → `/grades/2nd-grade/vowel-teams-oi-oy` | Implemented | Learn the spellings in boy and coin. |
| 4 | Core Spelling | **Diphthongs: ou and ow** | Direct Grade Unit: `vowel-teams-ou-ow` → `/grades/2nd-grade/vowel-teams-ou-ow` | Implemented | Practice the sound in out and cow. |
| 5 | Core Spelling | **Two Sounds of oo** | Direct Grade Unit: `grade-2-oo-two-sounds` → `/grades/2nd-grade/oo-two-sounds` | Implemented | Contrast moon and book. |
| 6 | Core Spelling | **Vowel Patterns: au and aw** | Direct Grade Unit: `grade-2-au-aw-words` → `/grades/2nd-grade/au-aw-words` | Implemented | Practice saw and because. |
| 7 | Core Spelling | **R-Controlled Vowels: ER, IR, and UR** | Direct Grade Unit: `grade-2-r-controlled-er-ir-ur` → `/grades/2nd-grade/r-controlled-er-ir-ur` | Implemented | Complete the basic r-controlled set. |
| 8 | Core Spelling | **Soft C and Soft G** | Direct Grade Unit: `grade-2-soft-c-soft-g` → `/grades/2nd-grade/soft-c-soft-g` | Implemented | Learn the changes in city and page. |
| 9 | Core Spelling | **Two-Syllable Words** | Direct Grade Unit: `grade-2-two-syllable-words` → `/grades/2nd-grade/two-syllable-words` | Implemented | Break longer words into two beats. |
| 10 | Core Spelling | **Words Ending in -le** | Direct Grade Unit: `grade-2-final-stable-le` → `/grades/2nd-grade/final-stable-le` | Implemented | Practice final stable -le. |
| 11 | Core Spelling | **Silent Letter Words** | Direct Grade Unit: `grade-2-silent-letter-words` → `/grades/2nd-grade/silent-letter-words` | Implemented | Spell knee and thumb. |
| 12 | Core Spelling | **Compound Words** | Direct Grade Unit: `grade-2-list-02` → `/grades/2nd-grade/compound-words` | Implemented | Build words from familiar parts. |
| 13 | Core Spelling | **Contractions** | Direct Grade Unit: `grade-2-contractions` → `/grades/2nd-grade/contractions` | Implemented | Join words with an apostrophe. |
| 1–6 | High-Frequency Words | **Grade 2 High-Frequency Words 1–6** | Direct canonical sets: `grade-2-high-frequency-words-set-1` through `-6` | Implemented | Practice 72 Grade 2 writing words. |
| 1–5 | Additional Practice | **Transportation, Money, Number Words 20–100, Community Helpers, Months** | Five bounded vocabulary pages | Implemented | Practice useful closed vocabulary sets. |

**Final corrective-pass decision:** EE/EA and IE/IGH are separate focused units placed before the advanced-vowel sequence. ER/IR/UR follows AU/AW and precedes Soft C/G. Grade 1 establishes AR/OR; the Grade 2 unit completes the r-controlled set, and Two-Syllable Words links only to the Multisyllabic Words Skill because it applies rather than teaches the vowel patterns.

**High-Frequency Words decision:** show all six direct set cards rather than only the gateway, matching the Kindergarten and Grade 1 convention. The gateway remains published for orientation, collection metadata, and a crawlable complete child list. The six sets contain 72 unique words with zero overlap with the 112 words already owned by the Kindergarten and Grade 1 High-Frequency Words sets.
...
The visible Grade 3 hub is implementation-complete. It has sixteen cards: seven Core Spelling cards, five direct High-Frequency Words set cards, and four Additional Practice cards. The `grade-3-high-frequency-words` collection remains the canonical explanatory gateway, but the hub exposes its five children directly, matching the Kindergarten, Grade 1, and Grade 2 convention. See the Grade 3 Additional Practice subsection below for the full record.

| # | Section | Public title | Type and destination | Status | Parent-facing purpose |
|---:|---|---|---|---|---|
| 1 | Core Spelling | **Prefixes** | Direct Grade Unit: `grade-3-prefix-words` → `/grades/3rd-grade/prefix-words` | Implemented | Learn common prefixes such as un-, re-, pre-, dis-, and mis- that come before a base word and change its meaning. |
| 2 | Core Spelling | **Suffixes** | Direct Grade Unit: `grade-3-suffix-words` → `/grades/3rd-grade/suffix-words` | Implemented | Practice common suffixes such as -er, -est, -ly, -ful, -less, -ness, and -ment that come after a base word. |
| 3 | Core Spelling | **Suffix Spelling Changes** | Direct Grade Unit: `grade-3-suffix-spelling-changes` → `/grades/3rd-grade/suffix-spelling-changes` | Implemented | Learn the three spelling changes a base word can make before a suffix: dropping silent e, doubling the final consonant, and changing y to i. |
| 4 | Core Spelling | **Plurals, Possessives, and Contractions** | Direct Grade Unit: `grade-3-possessives` → `/grades/3rd-grade/possessives` | Implemented | Use 's and s' to show ownership, and tell possessives apart from plurals and contractions. |
| 5 | Core Spelling | **Longer Words and Syllable Division** | Direct Grade Unit: `grade-3-multisyllabic-words` → `/grades/3rd-grade/multisyllabic-words` | Implemented | Break two- and three-syllable words into parts so they are easier to spell. |
| 6 | Core Spelling | **Homophones and Commonly Confused Words** | Direct Grade Unit: `grade-3-homophones` → `/grades/3rd-grade/homophones` | Implemented | Choose the correct spelling for words that sound alike but have different meanings, such as there, their, and they're. |
| 7 | Core Spelling | **Root Word Families** | Direct Grade Unit: `grade-3-root-word-families` → `/grades/3rd-grade/root-word-families` | Implemented | Review how related words share a meaningful base, and preview the Greek and Latin root study that begins in Grade 4. |
| 1–5 | High-Frequency Words | **Grade 3 High-Frequency Words 1–5** | Direct canonical sets: `grade-3-high-frequency-words-set-1` through `-5` | Implemented | Practice 60 Grade 3 writing words in five cumulative sets covering position/connecting words, irregular past-tense verbs, reading/writing vocabulary, feelings and description, and time/transition words. |

**Suffix Spelling Changes decision:** one coherent Grade Unit at `/grades/3rd-grade/suffix-spelling-changes` explicitly and substantially teaches dropping final e, doubling the final consonant, and changing final y to i. The reusable `suffix-spelling-changes` Skill supplies the grade-independent reference; no separate Grade Units are created for doubling or y-to-i.
...
**High-Frequency Words decision:** show all five direct set cards rather than only the gateway, matching the Kindergarten, Grade 1, and Grade 2 convention. The five sets contain 60 words with zero overlap with the 184 words already owned by the sixteen Kindergarten, Grade 1, and Grade 2 High-Frequency Words sets. `grade-2-high-frequency-words-set-6`'s `nextLists` now points to `grade-3-high-frequency-words-set-1`, closing the sequence gap noted in Section 10.

**Additional Practice decision (superseded):** the original implementation pass evaluated three candidate topics — Calendar Words, Clothing Words, and Transportation Words — against the bounded-set quality bar used at every prior grade, and none cleared it: Calendar Words did not add enough educational value to justify a permanent page, and Clothing Words and Transportation Words overlapped earlier instruction and common vocabulary already covered elsewhere. Grade 3 shipped with zero Additional Practice cards on that basis. That section explicitly flagged the decision as provisional, not permanent — "Additional Practice may be revisited once the full K–5 curriculum is complete, when broader cross-grade enrichment opportunities can be evaluated together across grades" — and a later editorial pass did exactly that.

**Additional Practice decision (current):** the later pass revisited the bar under a broader philosophy: a topic no longer needs to trace to a named academic standard, only to be a recognizable, bounded, non-duplicative theme. Under that revised bar, four new Grade 3 topics were approved and shipped: Map & Globe Words, Life Cycle Words, Time Words, and Multiplication & Division Words (`grade-3-map-globe-words`, `grade-3-life-cycle-words`, `grade-3-time-words`, `grade-3-multiplication-division-words`, registered in `GRADE_3_HUB_SECTIONS` in `src/lib/content/gradeHubCards.ts`). None of the four overlaps the three originally-rejected candidates (Calendar, Clothing, Transportation) or any Grade 3 Core Spelling or High-Frequency Words content. This document's earlier "no Additional Practice cards" framing is corrected here to match the live implementation.

### Grade 3 Additional Practice

Four cards, each a bounded, recognizable vocabulary theme with no overlap against Grade 3 Core Spelling or High-Frequency Words content:

| Card | Words | Theme |
|---|---|---|
| Map & Globe Words | compass, continent, ocean, border, region, direction, scale, legend | Map-skills vocabulary. |
| Life Cycle Words | egg, larva, caterpillar, cocoon, pupa, tadpole, hatch, cycle | Life-cycle-stage vocabulary. |
| Time Words | hour, minute, second, o'clock, digital, analog, clock, elapsed | Clock and time vocabulary. |
| Multiplication & Division Words | multiply, divide, product, factor, multiple, quotient, array, equation | Math-operations vocabulary; closes out the Grade 3 Additional Practice section. |

## 8. Exact Grade 4 grade-hub cards

The visible Grade 4 hub is implementation-complete. Core Spelling has six cards. High-Frequency Words has four direct numbered sets (48 words); the `grade-4-high-frequency-words` gateway remains the canonical explanatory landing page. Additional Practice has four cards: Measurement Words, Solar System Words, Career & Occupation Words, and Geometry Words. Executable route and Hub manifests are authoritative for current membership.

| # | Section | Public title | Type and destination | Underlying content | Status | Parent-facing purpose |
|---:|---|---|---|---|---|---|
| 1 | Core Spelling | **Multisyllabic Academic Words** | Direct Grade Unit: `grade-4-multisyllabic-academic-words` → `/grades/4th-grade/multisyllabic-academic-words` | Existing page, reframed. | Implemented | Break longer academic words such as *communicate* and *organize* into syllables before moving into affixes and roots. |
| 2 | Core Spelling | **Advanced Prefix Words** | Direct Grade Unit: `grade-4-advanced-prefixes` → `/grades/4th-grade/advanced-prefixes` | Existing page, reframed. | Implemented | Learn prefixes such as inter-, sub-, super-, trans-, and anti- that appear in longer academic words. |
| 3 | Core Spelling | **Advanced Suffix Words** | Direct Grade Unit: `grade-4-advanced-suffixes` → `/grades/4th-grade/advanced-suffixes` | Existing page, reframed. | Implemented | Practice -able, -ible, -ous, -ive, -tion, and -sion in longer Grade 4 words. |
| 4 | Core Spelling | **Latin Root Words** | Direct Grade Unit: `tier-1-roots-and-patterns` → `/grades/4th-grade/roots-and-patterns` | Existing page, reframed. | Implemented | Use port, dict, spect, rupt, and struct as spelling anchors in longer words. |
| 5 | Core Spelling | **Commonly Confused Words** | Direct Grade Unit: `grade-4-commonly-confused-words` → `/grades/4th-grade/commonly-confused-words` | Existing page, reframed. | Implemented | Choose the right spelling for words that sound alike or look similar, such as its/it's, than/then, and affect/effect. |
| 6 | Core Spelling | **Derived Words and Word Meaning** | Direct Grade Unit: new page | New content. | Implemented | Use recognizable spelling relationships across families such as nation, national, and nationality, including joins that adjust. |

**Card order decision:** Multisyllabic Academic Words opens the sequence, then Advanced Prefix Words, Advanced Suffix Words, Latin Root Words, Commonly Confused Words, and Derived Words and Word Meaning. The table uses exact canonical frontmatter titles; older conceptual roadmap labels do not override those titles. The remainder of this paragraph records the sequencing rationale. Structured-literacy sources (Fundations Level 4, Words Their Way's Derivational Relations stage, UFLI) consistently sequence affix work before Greek/Latin root work: prefixes and suffixes have a clear positional job around a word, while recurring roots require more abstract attention to meaningful internal spelling units, making formal root study the later-arriving skill — consistent with the canonical curriculum document's framing of Grade 4 as the point of *formal* root study. This also keeps Roots positioned after Multisyllabic Words, since root recognition is applied inside multisyllabic decoding rather than before it. The closing position is independently motivated: Grade 3's own last card (Root Word Families) established the precedent that the final Core Spelling card bridges into the next grade's deepest concept, while an accuracy/usage card (Homophones/Confused Words) sits second-to-last as a polish step. Derived Words and Word Meaning closes Grade 4 for the same reason Root Word Families closed Grade 3 — Grade 5's frozen map (Section 3) explicitly adds "related-word spellings **new**," building on Derived Words, not on Confused Words.

**Title decision:** canonical frontmatter titles are the source for Grade Hub card titles unless an explicit reviewed Hub-only override exists. Grade 4 therefore uses **Multisyllabic Academic Words**, **Advanced Prefix Words**, **Advanced Suffix Words**, **Latin Root Words**, **Commonly Confused Words**, and **Derived Words and Word Meaning**. Conceptual roadmap labels such as “Greek and Latin Roots” describe broader curriculum planning and do not replace page identity.

**Advanced Suffix Words current disposition:** the former `grade-4-final-stable-syllables` related-practice component was deleted with no content migration, and the anchor relationship was removed. The active canonical Grade Unit covers only its frozen -able, -ible, -ous, -ive, -tion, and -sion list.

**Latin Root Words current disposition:** the former `tier-2-greek-latin-roots` sibling was removed from the active architecture. The canonical Grade Unit is `tier-1-roots-and-patterns`, whose frozen title is **Latin Root Words** and whose list practices port, dict, spect, rupt, and struct. Its `greek-and-latin-roots` Skill relationship remains the deeper reference; that Skill title does not replace the Grade Unit title.
...
**Derived Words and Word Meaning decision:** this is the one wholly new Grade 4 Core Spelling page. It extends Grade 3's introductory family noticing by using a known family member to spell another, preserving recognizable shared letters while checking suffixes and joins separately. The frozen families are `nation/national/nationality`, `muscle/muscular`, `human/humanity`, `popular/popularity`, and `origin/original/originate`; `muscle → muscular` makes explicit that a join may adjust rather than every base letter remaining unchanged. The card closes the Grade 4 sequence and previews Grade 5's related-word spelling unit.
...
Gateway: `grade-4-high-frequency-words` collection (`category: high-frequency-words`, `grade: "4"`), same model as `grade-3-high-frequency-words`. Four `high-frequency-word-set` entries, `relatedLists: []` (matching the Grade 2/3 convention — no Heart Words page to link). Set 1's `prerequisiteLists` closes the gap from `grade-3-high-frequency-words-set-5`, the same way `grade-2-high-frequency-words-set-6` → `grade-3-high-frequency-words-set-1` was closed.

### Grade 4 Additional Practice

Four current cards are registered by the executable Hub manifest: **Measurement Words**, **Solar System Words**, **Career & Occupation Words**, and **Geometry Words**. The earlier one-card decision below is retained only as historical rationale for Measurement Words and does not define current membership. **Measurement Words** uses customary units only — `inch, foot, yard, mile, ounce, pound, ton, cup, pint, quart, gallon` (11 words). Tied to CCSS 4.MD.A.1 (the first point in the K–5 math sequence requiring students to know and convert measurement units), which no K–3 Additional Practice card touches. Verified zero overlap with Grade 4 Core Spelling, High-Frequency Words, or off-hub vocabulary; `cup` appears once, incidentally, in a Kindergarten CVC phonics example list, which is an allowed non-conflicting appearance under the same rule that governs incidental High-Frequency Words appearances.

**Placeholder-rejection decision:** this document's earlier Grade 4 Additional Practice row read "Math; Science; Social Studies; opinion/explanation writing after audit" — a placeholder, not a frozen decision (Section 1, "What remains flexible"). Audited against the same bounded-set and no-duplication rules every prior grade's Additional Practice candidates were held to, none of it survived: "Math" and "Science" are open, ever-growing domains, the same defect that sank Grade 2's "Math Words" candidate (Section 6); "Social Studies" would duplicate Grade 4's own off-hub `4th-grade-community-words.md` almost word-for-word; "opinion/explanation writing" would duplicate the Grade 4 High-Frequency Words Set 4 theme above. Measurement Words is the one candidate that clears the bar, using the same rescue pattern that saved Grade 2's Money Words from the "Math Words" rejection — bound a math-adjacent domain to the exact closed set a specific grade-level standard names, rather than the open subject as a whole.
...
Rejected candidates and why: Compass Directions (reads as "geography," excluded by Section 1's "Deferred optional content" rule, and thin); Ordinal Words (`first` is Grade 1 High-Frequency Words, `eighth` is Grade 4's own off-hub vocabulary); Weather Words (open domain, and overlaps the `weather`/`whether` pair in High-Frequency Words Set 4); Calendar/Clothing/Transportation Words (already rejected at Grade 3 for the same reasons, Section 7); Family/School vocabulary (explicitly named in Section 1's blocklist).

## 9. High-Frequency Words content model and Heart Word treatment
...
| Need | Representation |
|---|---|
| Gateway | `spelling-collections` entry, `category: high-frequency-words`, grade set, ordered `listIds`. |
| Numbered set | `spelling-lists` entry with `contentRole: high-frequency-word-set`, `category: high-frequency-words`, grade, stable ID, `order`, and `sourceType: curated`. |
...
| Provenance | `canonicalSource` records `Hybrid editorial` with a tier such as `K high-frequency words`; body copy names Dolch/Heart Word source overlap without claiming that a set reproduces a source list. |
...
|---|---|---|---|
| K gateway | Kindergarten High-Frequency Words to Spell | `Kindergarten High-Frequency Words to Spell: 4 Sets | spellingwords` | “Includes kindergarten high-frequency words and Heart Word guidance.” |
| K set | Kindergarten High-Frequency Words 1 | `Kindergarten High-Frequency Words 1 | High-Frequency Words and Spelling` | Explain the set’s phonics-aware progression; do not repeat the gateway’s full copy. |
| Grade 1 gateway | Grade 1 High-Frequency Words to Spell | `1st Grade High-Frequency Words to Spell: 6 Sets | spellingwords` | “Includes first-grade high-frequency-word and Heart Word support.” |
| Grade 1 set | Grade 1 High-Frequency Words 1 | `1st Grade High-Frequency Words 1 | High-Frequency Words and Spelling` | Use “common spelling words” naturally. |
| Grade 2 gateway | Grade 2 High-Frequency Words to Spell | `2nd Grade High-Frequency Words to Spell: 6 Sets | spellingwords` | “Includes second-grade high-frequency words and Heart Word guidance.” |
| Grade 2 set | Grade 2 High-Frequency Words 1 | `Grade 2 High-Frequency Words 1 | High-Frequency Words and Spelling` | Use “common spelling words” naturally. |
...
## 10. Implemented Kindergarten, Grade 1, Grade 2, and Grade 3 High-Frequency Words content

### Gateways

| Title | Stable ID | Route | Members | Parent-facing introductory copy |
|---|---|---|---|---|
| Kindergarten High-Frequency Words | `kindergarten-high-frequency-words` | `/grades/kindergarten` | `kindergarten-high-frequency-words-set-1` through `-4`; 40 words total | “These four small sets help kindergarteners spell everyday words used in early reading and writing. Most words can be connected to sounds your child knows; when one small part does not match yet, the page points it out as a Heart Word part.” |
| Grade 1 High-Frequency Words | `grade-1-high-frequency-words` | `/grades/1st-grade` | `grade-1-high-frequency-words-set-1` through `-6`; 72 unique words total | “These six cumulative sets build useful Grade 1 writing words without treating them as a list to memorize blindly. Children use familiar phonics when possible and learn only the unexpected spelling parts by heart.” |
| Grade 2 High-Frequency Words | `grade-2-high-frequency-words` | `/grades/2nd-grade` | `grade-2-high-frequency-words-set-1` through `-6`; 72 unique words total | “These six sets carry a second grader through the last heavy year of high-frequency-word learning. Most of these words can now be sounded out with the patterns your child already knows — vowel teams, syllables, and silent letters. Where one small part is still unexpected, the page names it so your child learns only that part by heart.” |
| Grade 3 High-Frequency Words | `grade-3-high-frequency-words` | `/grades/3rd-grade` | `grade-3-high-frequency-words-set-1` through `-5`; 60 words total | “These five sets carry a third grader through everyday writing words and persistent trouble words: words that no longer sound out with a single phonics rule and instead need to be recognized, used correctly in context, or partly remembered. Where a word repeats a pattern already flagged as a Heart Word earlier in the K–2 sequence (such as an -ough family word), the set names the connection instead of re-teaching it as new.” |
...
|---|---|---|---|---|
| Kindergarten High-Frequency Words 1 | `kindergarten-high-frequency-words-set-1`<br>`/grades/kindergarten/high-frequency-words-1` | **a, I, am, at, can, in, it, is, and, the** (10) | a T; I T; am R; at R; can R; in R; it R; is T; and R; the P (`e`). | Hybrid early-writing set drawing on Dolch Pre-Primer A/B/C and existing K Heart Words. `am/at/can/in/it` are incidental phonics examples; none has another High-Frequency Words membership. First set; next `-2`. Intro: “Start with ten short words children use to label, answer, and make simple sentences. Sound out the parts that work as expected, then notice the small tricky parts in a, I, is, and the.” |
| Kindergarten High-Frequency Words 2 | `kindergarten-high-frequency-words-set-2`<br>`/grades/kindergarten/high-frequency-words-2` | **he, she, we, me, my, go, to, do, you, like** (10) | he T; she T; we T; me T; my T; go T; to P (`o`); do P (`o`); you P (`ou`); like T. | Hybrid Dolch Pre-Primer/Primer plus existing K Heart Words. These are intentionally not copied into a second numbered set. Prerequisite `-1`; next `-3`. Intro: “Use these common people, action, and sentence words to write about yourself and others. Many become easier to explain as children learn long-vowel patterns; to, do, and you still need a remembered part now.” |
| Kindergarten High-Frequency Words 3 | `kindergarten-high-frequency-words-set-3`<br>`/grades/kindergarten/high-frequency-words-3` | **for, of, was, said, have, are, here, come, look, see** (10) | for T; of P (`f`); was P (`a`); said P (`ai`); have T; are T; here T; come P (`o`); look T; see T. | Hybrid Dolch and K Heart Words; connects incidentally to silent-e/vowel-team knowledge taught later. Prerequisite `-2`; next `-4`. Intro: “Practice high-use words that help children explain, tell, and describe. Use the sounds that make sense and learn the small unexpected parts in words such as said, was, of, and come.” |
| Kindergarten High-Frequency Words 4 | `kindergarten-high-frequency-words-set-4`<br>`/grades/kindergarten/high-frequency-words-4` | **this, that, with, they, one, two, three, where, little, play** (10) | this T; that T; with T; they T; one P (`o_e`); two P (`w`); three P (`ee`); where P (`wh/ere`); little T; play T. | Hybrid Dolch Pre-Primer/Primer and existing K topical material (`one`, `two`, `three`, `play`). Topic-page appearances are incidental; this set owns high-frequency-word membership. Prerequisite `-3`; no next K set; related to Grade 1 gateway. Intro: “Finish kindergarten high-frequency-word practice with words used in early questions, counting, and simple stories. Several spellings are not fully predictable yet, so practice the regular parts and remember only the tricky part.” |
...
|---|---|---|---|
| `grade-1-high-frequency-words-set-1` | all, but, did, no, get, good, new, now, our, out, please, want | Next: set 2 | Useful words for sentences, directions, and everyday writing. |
| `grade-1-high-frequency-words-set-2` | after, again, any, ask, by, could, every, fly, from, give, going, had | Previous: set 1; next: set 3 | Words for stories, questions, and classroom writing. |
| `grade-1-high-frequency-words-set-3` | on, not, an, as, if, has, his, her, him, them, be, will | Previous: set 2; next: set 4 | Joining words, pronouns, action words, and helping words. |
| `grade-1-high-frequency-words-set-4` | what, when, who, why, how, there, your, their, were, some, more, because | Previous: set 3; next: set 5 | Questions, explanations, comparisons, and reasons. |
| `grade-1-high-frequency-words-set-5` | up, down, back, over, into, about, home, way, time, first, next, then | Previous: set 4; next: set 6 | Place, movement, time, and sequence words. |
| `grade-1-high-frequency-words-set-6` | or, so, just, us, may, make, many, very, people, know, would, should | Previous: set 5 | Choices, explanations, opinions, and fluent writing. |
...
All five sets are published, contain 12 words each, and form a live adjacent sequence (set 1's prerequisite is `grade-2-high-frequency-words-set-6`; set 5 has no next set). Word membership is unique across the sequence and has zero overlap with the 184 words owned by the sixteen Kindergarten, Grade 1, and Grade 2 sets. `relatedLists` is `[]` on every set, matching the Grade 2 convention — the sequence relationship lives entirely in `prerequisiteLists`/`nextLists`.

| Set | Ordered words | Sequence relationship | Editorial focus |
|---|---|---|---|
| `grade-3-high-frequency-words-set-1` | across, above, below, beneath, beside, among, upon, toward, against, within, beyond, throughout | Prerequisite: `grade-2-high-frequency-words-set-6`; next: set 2 | Position and connecting words; groups *throughout* with the `ough` family already taught in Grade 2's *through* and *enough*. |
| `grade-3-high-frequency-words-set-2` | brought, bought, caught, taught, fought, sought, built, heard, earned, paid, laid, weighed | Previous: set 1; next: set 3 | Irregular past-tense verbs; groups the six shared `-ought`/`-aught` words as one family. |
| `grade-3-high-frequency-words-set-3` | library, dictionary, paragraph, subject, chapter, author, title, describe, explain, practice, opinion, nonfiction | Previous: set 2; next: set 4 | Reading and writing vocabulary for talking about books and student writing. |
| `grade-3-high-frequency-words-set-4` | favorite, wonderful, terrible, awful, difficult, especially, excited, nervous, curious, jealous, proud, disappointed | Previous: set 3; next: set 5 | Feelings and description words with more precision than *happy* or *sad*. |
| `grade-3-high-frequency-words-set-5` | finally, probably, actually, immediately, eventually, meanwhile, instead, otherwise, therefore, certainly, definitely, recently | Previous: set 4; no next set | Time and transition words that sequence ideas and connect sentences in writing. |

The gateway's introductory copy and each set's body copy are quoted in full in the content entries; this table freezes architecture, sequence, and word membership.

## 11. Completed K–5 implementation

Kindergarten, Grade 1, Grade 2, Grade 3, Grade 4, and Grade 5 now all realize the three-section architecture on their public hubs. Grade 3's Additional Practice section shipped in two passes: an initial review rejected three candidate topics and shipped zero cards, and a later pass under a broader philosophy approved and shipped four different topics — Map & Globe, Life Cycle, Time, and Multiplication & Division Words (Section 7). All six High-Frequency Words gateways and all twenty-nine K–5 child sets are published. Kindergarten exposes four sets and 40 words; Grade 1 and Grade 2 each expose six sets and 72 unique words; Grade 3 exposes five sets and 60 words; Grade 4 and Grade 5 each expose four sets and 48 words. The public hubs link directly to their set cards while their collection gateways remain canonical explanatory landing pages.

Grade 4's Core Spelling section realizes the frozen hub pattern: six cards, two of them combined roadmap entries reusing existing pages, one wholly new closing page (Section 8). Grade 4's High-Frequency Words (four sets, 48 words) and Additional Practice (one card, Measurement Words) are also now implemented (Section 8), following the same audit process as every prior grade's Additional Practice decision — including, for Additional Practice, arriving at exactly one card rather than the three K–2 shipped (Grade 3's Additional Practice count was revised in a later pass — see Section 7).

Grade 5's Core Spelling section also now realizes the hub pattern: five cards, including one wholly new closing page (Section 14). Grade 5's High-Frequency Words are also implemented: four sets, 48 words, zero overlap with the 292 words already owned by Kindergarten–Grade 4 (Section 15). Grade 5's Additional Practice now contains four cards: Money Management, Ecosystem & Environment, Fraction & Decimal, and Civics and Government Words (Section 16). The Grade 5 hub now renders all three sections — Grade 5 is implementation-complete, with no phase remaining deferred.

The completed implementation preserves existing flat canonical routes, stable IDs, parent/adjacent links, server-rendered gateway orientation, and visible-content-aligned breadcrumb and `ItemList` structured data. Focused Skills and compatibility pages retain their routes without becoming duplicate canonical curriculum pages. Grade 2's, Grade 3's, Grade 4's, and Grade 5's implementations additionally required no schema change — they reused `contentRole: grade-unit`/`skill`/`high-frequency-word-set`/`vocabulary-theme`, `skillIds`, `prerequisiteLists`/`nextLists`, and the existing `spelling-collections` gateway model exactly as documented above.
...
- Publish one High-Frequency Words gateway per grade and the set counts frozen in Section 1.
...
- Use natural familiar terms (“high-frequency words,” “Heart Words,” “high-frequency words”) in explanatory copy and metadata while retaining the one High-Frequency Words architecture.
...
1. ~~Implement and validate Grade 2 Core Spelling and High-Frequency Words.~~ **Done.** The final corrective pass adds focused EE/EA, IE/IGH, and ER/IR/UR units, bringing Grade 2 to thirteen Core Spelling cards; see Section 6.
2. ~~Normalize Grade 3’s suffix-change and syllable-division route.~~ **Done.** Grade 3 shipped its full three-section hub: seven Core Spelling cards (including the merged Suffix Spelling Changes combined roadmap entry and the new Possessives page), five High-Frequency Words sets, and — after an initial reviewed-and-rejected pass, later revisited under a broader philosophy — four Additional Practice cards (Map & Globe, Life Cycle, Time, and Multiplication & Division Words) — see Section 7.
...
4. ~~Curate Grade 4's narrower High-Frequency Words sequence and Additional Practice.~~ **Done.** Grade 4 shipped four High-Frequency Words sets (48 words) and one Additional Practice card (Measurement Words) — see Section 8.
...
6. ~~Curate Grade 5's High-Frequency Words sequence.~~ **Done.** Grade 5 shipped four High-Frequency Words sets (48 words), zero overlap with the 292 words already owned by Kindergarten–Grade 4 — see Section 15.
...
8. Add optional topic cards only after each grade’s core route and High-Frequency Words system are complete and the topics clear editorial review. All six K–5 grades are now implementation-complete; no grade has a pending phase.
...
| # | Section | Public title | Type and destination | Underlying content | Status | Parent-facing purpose |
|---:|---|---|---|---|---|---|
| 1 | Core Spelling | **Advanced Multisyllabic Academic Words** | Direct Grade Unit: `grade-5-multisyllabic-academic-words` → `/grades/5th-grade/multisyllabic-academic-words` | Existing page, reframed. | Implemented | Break longer academic words such as *investigation* and *responsibility* into syllables and word parts before moving into affixes and roots. |
| 2 | Core Spelling | **Advanced Prefixes and Suffixes** | Combined roadmap entry: `grade-5-prefix-suffix-words` (anchor) → `/grades/5th-grade/prefix-suffix-words`, with `grade-5-spelling-rules` (Spelling Rules for Word Endings) as related practice | Existing anchor and sibling, both reframed. | Implemented | Combine prefixes such as inter-, trans-, and super- with suffixes such as -ive, -ible, and -ity, then extend into the -tion/-sion/-able/-ible/-ance/-ence spelling rules that complete the set. |
| 3 | Core Spelling | **Greek and Latin Word Parts** | Direct Grade Unit: `grade-5-greek-latin-word-parts` → `/grades/5th-grade/greek-latin-word-parts` | Existing page, reframed. | Implemented | Use familiar Greek and Latin word parts such as photo, bio, geo, port, and struct inside longer Grade 5 academic vocabulary. |
| 4 | Core Spelling | **Commonly Confused Words** | Direct Grade Unit: `grade-5-commonly-confused-words` → `/grades/5th-grade/commonly-confused-words` | Existing page, reframed. | Implemented | Choose the right spelling for words that sound alike or look similar, such as affect/effect, principal/principle, and advice/advise. |
...
**Advanced Prefixes and Suffixes decision:** Grade 5 retains one combined hub card anchored on `grade-5-prefix-suffix-words`; `grade-5-spelling-rules` remains reachable through its exact pre-PR relationship and progression metadata. The sibling's stored `contentRole: skill` is legacy metadata, not canonical taxonomy membership — that question is settled, not open. The value is kept deliberately as a documented exception because reclassifying it to `grade-unit` was found to change its live rendering (see `docs/content/inventory/skill-pages.md`); only the frontmatter's final technical normalization is deferred to the Grade curriculum editorial phase. It is not added to the Skills Hub or Grade Hub by this decision.
...
The audit also caught real defects in the pre-existing civics content that had never been checked against the live word banks: `government` and `president` were already Grade 4 High-Frequency Words (Sets 1 and 3), and `responsibility` was already a Grade 5 Core Spelling word (Advanced Multisyllabic Academic Words) — all three replaced (`amendment`, `congress`, `liberty` respectively). During implementation, a fourth collision was found and fixed the same way: `democracy` was already a Grade 4 Core Spelling word (`tier-2-greek-latin-roots`, a Greek-root example), replaced with `justice`. This is exactly the kind of problem the "remain unwired pending research" holding pattern (Section 14) existed to catch.
...
**Zero-overlap verification:** both new/corrected cards' 24 words were checked programmatically against every word in every published K–5 High-Frequency Words set and every K–5 Core Spelling (`contentRole: grade-unit`/`skill`) page — zero overlaps remain. This check is now a standing regression test (`gradeHubCards.test.ts`).

## 15. Exact Grade 5 High-Frequency Words (High-Frequency Words)

Four cumulative 12-word sets, 48 words total, zero overlap with the 292 words already owned by the twenty-nine Kindergarten, Grade 1, Grade 2, Grade 3, and Grade 4 High-Frequency Words sets (verified directly against every published set file's `words:` array, not from memory — 340 total K–5 words, all unique). Matches the frozen 4-set count (Section 1) and the 12-words-per-set convention every implemented grade but Kindergarten has used.
...
**Rejected words and why:** `especially` was proposed during review and rejected outright — it is already a Grade 3 High-Frequency Words word (`grade-3-high-frequency-words-set-4`), a zero-overlap violation, not an editorial judgment call. `guard` was an earlier Set 1 candidate, replaced with `vague`: `guard`'s silent-u pattern (`gu-`) is already familiar from earlier, simpler high-frequency words (guess, guest, guide), making it a weaker "narrow corrective" pick by Grade 5 than the rest of the set; `vague`'s silent-u-before-e pattern stays genuinely unpredictable well past elementary school and is a stronger Tier 2 fit for opinion/argument writing. `Wednesday` and `February` were both considered and dropped for Set 1 and Set 3 respectively — not because they overlap any High-Frequency Words set, but because Grade 1's and Grade 2's existing Additional Practice cards ("Days of the Week," "Months of the Year") almost certainly already own them, and a parent should not see the same word twice across the site.

Gateway: `grade-5-high-frequency-words` collection (`category: high-frequency-words`, `grade: "5"`), same model as `grade-4-high-frequency-words`. Four `high-frequency-word-set` entries, `relatedLists: []` (matching the Grade 2–4 convention — no Heart Words page to link). Set 1's `prerequisiteLists` closes the gap from `grade-4-high-frequency-words-set-4`, the same way `grade-3-high-frequency-words-set-5` → `grade-4-high-frequency-words-set-1` was closed; `grade-4-high-frequency-words-set-4.nextLists` was updated from `[]` to `["grade-5-high-frequency-words-set-1"]` to close that gap forward. `grade-5-high-frequency-words-set-4.nextLists: []` is the true end of the K–5 High-Frequency Words sequence.
