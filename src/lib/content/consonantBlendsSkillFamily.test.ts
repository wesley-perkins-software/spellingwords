...
import { CONSONANT_BLENDS_SKILL_FAMILY, CURATED_SPELLING_SKILL_IDS } from './spellingSkills';

const contentRoot = join(process.cwd(), 'src/content/spelling-lists');

const CONSONANT_BLENDS_SKILL_IDS = ['beginning-blends', 'ending-blends'] as const;
...
describe('Consonant Blends Skill Family', () => {
  it('uses exactly Beginning Blends and Ending Blends, in that order', () => {
    expect(CONSONANT_BLENDS_SKILL_FAMILY.skillIds).toEqual(CONSONANT_BLENDS_SKILL_IDS);
  });

  it('marks both blend Skills as reusable, published Skills', () => {
    for (const id of CONSONANT_BLENDS_SKILL_IDS) {
      expect(byId.get(id), id).toMatchObject({ id, contentRole: 'skill', status: 'published' });
    }
  });

  it('keeps stable ids, categories, and slugs', () => {
    expect(byId.get('beginning-blends')).toMatchObject({
      category: 'phonics',
      urlSlug: 'beginning-blends',
    });
    expect(byId.get('ending-blends')).toMatchObject({
      category: 'phonics',
      urlSlug: 'ending-blends',
    });
  });

  it('keeps each Skill demonstration word set within the 8-16 word range', () => {
    for (const id of CONSONANT_BLENDS_SKILL_IDS) {
  ...
  it('does not put rigid next-step sequencing on Consonant Blends Skills', () => {
    for (const id of CONSONANT_BLENDS_SKILL_IDS) {
