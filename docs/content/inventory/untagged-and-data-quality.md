...
**Resolved and moved:** `silent-e-long-e` (Long E Silent E Words) was previously listed here as one of the three non-taxonomy `skill`-tagged pages. It is now resolved per `docs/architecture/SKILLS_ARCHITECTURE.md` §5 — folded into the Silent E family's normal guidance sentence on the Skills Hub (no separate block, matching every other family's presentation) at `/skills#silent-e-family`, and set to `status: archived` so no standalone page is emitted. Its former repository-shaped path is not generated (the site is pre-launch, so there is no historical traffic to redirect). Moved to `deprecated-and-legacy-pages.md`'s "Archived-status pages" table; no longer counted in the unresolved pages above.

**Resolved (not moved — see `skill-pages.md`):** `grade-4-final-stable-syllables` and `grade-5-spelling-rules` were previously the other two rows of the non-taxonomy `skill`-tagged trio and are no longer listed as unresolved above. Their canonical-Skill-taxonomy question is settled: neither is, or will become, a canonical Skill. Both are grade-owned focused component pages of an existing combined Grade Hub unit (see the "Focused component page" notes in `grade-curriculum-pages.md`), reachable only via their anchor Grade Unit's related-practice link. Their stored `contentRole: skill` value is retained as a documented legacy-role exception rather than corrected to `grade-unit`, because reclassifying was found to route them through the `GradeUnitWorldPage` template and the cross-grade world-kit sequence in `src/lib/content/gradeUnitSequence.ts`, changing their live public rendering beyond a metadata fix. No new `contentRole` value was introduced. Full detail and rationale in `skill-pages.md`'s "Resolved: legacy-role exception" section. Final technical normalization of the frontmatter value is deferred to the Grade 4 and Grade 5 curriculum editorial passes.
...
`grade-4-final-stable-syllables`, `grade-5-spelling-rules` — two published pages tagged as Skill pages that are not part of `SKILLS_ARCHITECTURE.md`'s taxonomy and are not linked from the Skills Hub or any Grade Hub. **Resolved, not unresolved** — see the "Resolved (not moved)" note above and `skill-pages.md` for full detail: their `contentRole: skill` value is a documented, deliberately retained legacy-role exception, not an open taxonomy question.

A third page in this category, `silent-e-long-e`, is also resolved (see the "Resolved and moved" note above) — it carries `status: archived`, not `published`, and has moved to `deprecated-and-legacy-pages.md`.
