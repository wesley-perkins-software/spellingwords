...
A Skill's demonstration is not a Practice Set (see §4) and does not itself launch the practice experience. It exists to teach the concept — hearing the sound, highlighting the shared pattern, comparing forms — and it must not be curated as a copy of any Grade Unit's assigned words or of the Skill's own practice bank. Purposeful word-level overlap is allowed when a word is genuinely one of the clearest examples of the concept (a word may anchor a Grade Unit's assigned set, a Skill's demonstration, and the Skill's practice bank simultaneously), but the demonstration as a whole must never duplicate a Grade Unit's full assigned set, the Skill's practice bank, or read as a second assignment.

A Skill also owns exactly one **Skill practice bank** (see §4): a canonical, grade-independent pool of practice items for the concept, distinct from both the demonstration and any Grade Unit's Practice Set. A user who wants to practice the concept directly can do so from the Skill's own bank; a user who wants grade-sequenced context continues to be routed, via the Skill's curriculum-placement links, to the Grade Unit(s) where it is actually assigned. These are two distinct, complementary paths: direct Skill practice and grade-sequenced curriculum practice.
...
A Skill's `words` are a demonstration (see §3), not a Practice Set, and never launch the practice experience directly. A Skill additionally owns one **Skill practice bank** — a canonical, grade-independent pool of practice items distinct from the demonstration and from any Grade Unit's Practice Set. The practice experience (`/play`) already draws from more than one canonical source — custom words, plus curated word sets exposed by several content identities, Grade Units among them (see §3.1's Practice Tool description and the identities in §3). A Skill's practice bank is an additional canonical source alongside these, not a replacement for them and not a ceiling on what other content identities may also expose. A Skill's bank is never forked per grade — a Skill referenced by multiple grades' `skillIds` still owns only one bank, matching the existing rule that no grade owns a Skill (Constitution §7). A Skill practice session is a selection from the bank, not necessarily the entire bank.

The 8–16-word guideline applies to a Grade Unit's Practice Set. A Skill's demonstration is deliberately smaller — the smallest set that clearly teaches the concept, not a target word count — and is capped well below Practice Set or Skill-practice-bank size so it cannot be mistaken for either. A Skill's practice bank is sized for genuine practice coverage of the concept and is not bound by the demonstration's small-set guidance or the Grade Unit's 8–16-word guideline; its size, structure, and any richer-than-plain-string item shape are defined in implementation-level documentation, not this model.
