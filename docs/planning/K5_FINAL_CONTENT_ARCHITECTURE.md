...
**Advanced Suffixes and Final Stable Syllables decision:** rather than author a separate "Word Endings" card, the canonical Final Stable Syllables and Common Word Endings unit is realized as a combined roadmap entry anchored on the existing `grade-4-advanced-suffixes` page, which already teaches -tion/-sion alongside -able/-ible/-ous/-ive. One grade-owned `contentRole: supporting-practice` sibling page covers the -ture/-sure endings the anchor does not, reached through the anchor's related-practice link. It is neither a canonical Browse-by-Skill destination nor a separate Grade Hub card. This avoids a near-duplicate "suffixes vs. endings" pair of cards that would be hard for a parent to tell apart.
...
**Advanced Prefixes and Suffixes decision:** rather than split prefixes and suffixes into two cards the way Grade 4 does (Advanced Prefixes; Advanced Suffixes and Final Stable Syllables), Grade 5 realizes its canonical "Advanced Roots, Affixes, and Academic Words" unit as one combined affix card, `grade-5-prefix-suffix-words`, already teaching both prefixes and suffixes together. `grade-5-spelling-rules` (the -tion/-sion/-able/-ible/-ance/-ence endings) uses `contentRole: supporting-practice` and is reached through the anchor's related-practice link; it is neither a canonical Browse-by-Skill destination nor a separate Grade Hub card. Both pages retain their stable identities and progression relationships.
